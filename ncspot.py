# -*- coding: utf-8 -*-
"""
Display song currently playing in ncspot.
...
Configuration parameters:
    ...
    format_down: define output if ncspot is not running
        (default 'ncspot not running')
    format_stopped: define output if ncspot is not playing
        (default 'ncspot stopped')
...
stopped
{'color': '#FF0000', 'full_text': 'ncspot stopped'}
"""
...
NCSPOT_CMD = """dbus-send --print-reply --dest=org.mpris.MediaPlayer2.ncspot
...
class Py3status:
    ...
    def _ncspot_cmd(self, action):
        return NCSPOT_CMD.format(cmd=action)
    ...
    def _get_text(self):
        ...
        try:
            self.__bus = bus.get_object(
                "org.mpris.MediaPlayer2.ncspot", "/org/mpris/MediaPlayer2"
            )
    ...
    def on_click(self, event):
        ...
        if button == self.button_play_pause:
            self.py3.command_run(self._ncspot_cmd("PlayPause"))
            sleep(0.1)
        elif button == self.button_next:
            self.py3.command_run(self._ncspot_cmd("Next"))
            sleep(0.1)
        elif button == self.button_previous:
            self.py3.command_run(self._ncspot_cmd("Previous"))
            sleep(0.1)
